...
export async function sendMessageToCustomer(booking: Booking, tenant: Tenant, message: string): Promise<void> {
  try {
    if (!resend) { console.warn('[email] sendMessageToCustomer skipped - no Resend client'); return }
    if (!booking.email) return
...
<body style="font-family:sans-serif;color:#0f172a;background:#f8fafc;margin:0;padding:32px 16px;">
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;">
    ...
    await resend.emails.send({
      from: FROM,
      to: booking.email,
      subject: `A message from ${tenant.name}`,
      html,
    })
  } catch (err) {
    console.error('[email] Failed to send message to customer:', err)
    throw err
  }
}
